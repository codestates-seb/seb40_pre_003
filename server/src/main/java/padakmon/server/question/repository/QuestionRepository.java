...
public interface QuestionRepository extends JpaRepository<Question, Long> {
    ...
    Page<Question> tagSearch(String query, Pageable pageable);

    @Query(value = "SELECT q FROM Question q INNER JOIN q.user u WHERE u.id = :query")
    Page<Question> userSearch(long query, Pageable pageable);
    ...
    Page<Question> answeredNumberSearch(int number, Pageable pageable);

    @Query(value = "SELECT q FROM Question q WHERE q.voteScore >= :number")
    Page<Question> scoreSearch(int number, Pageable pageable);
